...
const slackManifest = {
  ...
  features: {
    bot_user: {
      display_name: "PreceptAIBot",
    },
  },
  oauth_config: {
    scopes: {
      bot: ["channels:history", "channels:join", "channels:read", "users:read"],
    },
  },
};
...
> {
  ...
  confirmDelete = (index: number) => {
    confirmAlert({
      title: "Alert",
      message: `Are you sure you want to delete ${this.capitilize(
        this.props.connectedDataSources[index].name
      )}?`,
      buttons: [
        {
          label: "Yes, delete it",
  ...
  render() {
    return (
      <div className="relative flex flex-col bg-[#fff] items-start px-8 pt-0 pb-4 min-h-[300px]">
        {!this.state.isAdding && (
          <h1 className="mt-4 relative self-center text-[#0D7E97] block text-4xl mb-8 font-dm-sans">
            Data Source Panel
          </h1>
        )}

        {/* X and Edit in top right */}
        <div className="absolute flex flex-col items-center right-4 top-3 text-2xl text-black gap-4">
          <IoMdClose
        ...
        {!this.state.isAdding && (
          <div className="w-full">
            <h1 className="text-2xl block text-black mb-4">
              {this.props.connectedDataSources.length > 0
                ? this.state.editMode
                  ? "Edit mode:"
                  : "Active data sources:"
                : "No Active Data Sources. Add Now!"}
              {this.props.connectedDataSources.length > 0 && (
                <FaRegEdit
                  key="pencil"
            ...
            <div className="flex flex-row w-[100%] flex-wrap">
                ...
                return (
                  ...
                  <div className="flex py-2 pl-5 pr-3 m-2 flex-row items-center justify-center bg-[#0D7E97] hover:shadow-inner shadow-blue-500/50 rounded-lg font-dm-sans leading-[28px] border-b-[#916CCD] border-b-2">
                    <img
                      ...
                      className={"mr-2 h-[20px]"}
                      src={
                        this.props.dataSourceTypesDict[dataSource.name]
                          .image_base64
                      }
                    ></img>
                    <h1 className="text-white width-full">
                      {
                        this.props.dataSourceTypesDict[dataSource.name]
                          .display_name
                      }
                    ...
                    {this.state.editMode ? (
                      this.state.removeInProgressIndex === index ? (
                        ...
                        <IoMdCloseCircle
                          ...
                          className="transition duration-150 ease-in-out  ml-6 fill-[#000] hover:cursor-pointer text-2xl hover:fill-[#fff]"
                        />
                      )
                    ) : (
              ...
              {Object.keys(this.props.dataSourceTypesDict).map((key) => {
                ...
                if (
                  !this.state.editMode &&
                  !this.props.connectedDataSources.find(
                    (connectedDataSource) =>
                      connectedDataSource.name === dataSource.name
                  )
                ) {
                  return (
                    // unconnected data source
                    <div
                      onClick={() => this.dataSourceToAddSelected(dataSource)}
                      className="flex hover:text-[#fff] py-2 pl-5 pr-3 m-2 flex-row items-center justify-center bg-[#0D7E97] hover:border-[#9875d4] rounded-lg font-dm-sans leading-[28px] border-[#777777] border-b-[.5px] transition duration-300 ease-in-out"
                    >
                      <img
                        alt=""
                        className={"mr-2 h-[20px]"}
                        src={dataSource.image_base64}
                      ></img>
          ...
          <div className="flex flex-col w-[100%] py-10">
            <div className="flex flex-row justify-left ml-2 items-center mb-5 mt-5">
              <img
                ...
                src={this.state.selectedDataSource.imageBase64}
              ></img>
              <Select
            ...
            </div>
            {
              ...
              <div className="flex flex-col ">
                <div className="bg-[rgba(0,0,0,0.06)] py-[26px] px-10 rounded-xl border-[1px] border-[#0D7E97]">
                  {this.state.selectedDataSource.value === "mattermost" && (
                    <span className="flex flex-col leading-9  text-xl text-black">
                      <span>
                        1.{" "}
                        {
                          "Go to your Mattermost -> top-right profile picture -> Profile"
                        }
                      </span>
                      <span>
                        2.{" "}
                        {
                          "Security -> Personal Access Tokens -> Create token -> Name it"
                        }
                      </span>
                      <span>3. {"Copy the Access Token"}</span>
                      <span className="text-violet-300/[.75] text-sm">
                        {" "}
                        {"* Personal Access Tokens must be on"} -{" "}
                        <a
                          className="inline hover:underline text-violet-400/[.75]"
                          target="_blank"
                          rel="noreferrer"
                          href="https://developers.mattermost.com/integrate/reference/personal-access-token/"
                        >
                          Click for more info
                        </a>
                      </span>
                  ...
                  {this.state.selectedDataSource.value === "confluence" && (
                    <span className="flex flex-col leading-9  text-xl text-black">
                      <span>
                        1.{" "}
                        {
                          "Go to your Confluence -> top-right profile picture -> Settings"
                        }
                      </span>
                      <span>
                        2. {"Personal Access Tokens -> Create token -> Name it"}
                      </span>
                      <span>
                        3.{" "}
                        {
                          "Uncheck 'Automatic expiry', create and copy the token"
                        }
                      </span>
                    </span>
                  )}
                  {this.state.selectedDataSource.value === "jira" && (
                    <span className="flex flex-col leading-9  text-xl text-black">
                      <span>
                        1.{" "}
                        {
                          "Go to your Jira -> top-right profile picture -> Settings"
                        }
                      </span>
                      <span>
                        2. {"Personal Access Tokens -> Create token -> Name it"}
                  ...
                  {(this.state.selectedDataSource.value === "jira_cloud" ||
                    ...
                    <span className="flex flex-col leading-9  text-xl text-black">
                      <span>
                        1. Go here:{" "}
                        <a
                          className="text-[#d6acff] hover:underline"
                          rel="noreferrer"
                          href={
                  ...
                  {this.state.selectedDataSource.value === "slack" && (
                    // slack instructions
                    <span className=" flex flex-col leading-9 text-lg text-black">
                      <span className="flex flex-row items-center">
                        1.
                        <button
                          onClick={this.copyManifest}
                          className="flex py-3 pl-3 pr-2 m-2 w-[110px] h-10 text-lg flex-row items-center justify-center bg-[#584971] active:transform active:translate-y-4 hover:bg-[#9875d4] rounded-lg font-dm-sans border-[#ffffff] border-b-[.5px] transition duration-300 ease-in-out"
                        >
                          <span>Copy</span>
                      ...
                      <span className="flex flex-row items-center">
                        {" "}
                        2. Create new app from manifest in &nbsp;
                        <a
                          className="text-[#d6acff] hover:underline"
                          rel="noreferrer"
                      ...
                      <span className="ml-8 mt-2">
                        <img
                          alt=""
                          className="h-[120px] rounded-xl p-1"
                          src={LeftPane}
                        />
                      </span>
                      ...
                      <span className="text-violet-300/[.75] mt-1 text-sm">
                        {" "}
                  ...
                  {this.state.selectedDataSource.value === "google_drive" && (
                    ...
                    <span className="leading-9 text-lg text-black">
                      Follow{" "}
                      <a
                  ...
                  {this.state.selectedDataSource.value === "bookstack" && (
                    <span className="flex flex-col leading-9  text-xl text-black">
                      <span>
                        1.{" "}
                        {
                          "Go to your Bookstack -> top-right profile picture -> Edit profile"
                        }
                      </span>
                      <span>
                        2.{" "}
                        {"Scroll down to API tokens -> Create token -> Name it"}
                      </span>
                      <span>
                        3.{" "}
                        {
                          "Set 'Expiry Date' 01/01/2100, create, copy token id + token secret"
                        }
                      </span>
                    </span>
                  )}
                  {this.state.selectedDataSource.value === "rocketchat" && (
                    <span className="flex flex-col leading-9  text-xl text-black">
                      <span>
                        1.{" "}
                        {
                          "In Rocket.Chat, click your profile picture -> My Account."
                        }
                      </span>
                      <span>2. {"Click Personal Access Tokens."}</span>
                      <span>
                        3. {'Check "Ignore Two Factor Authentication".'}
                      </span>
                      ...
                      <p>
                        Note that the url must begin with either http:// or
                        https://
                      </p>
                    </span>
                  )}
                  {this.state.selectedDataSource.value === "gitlab" && (
                    <span className="flex flex-col leading-9  text-xl text-black">
                      <span>
                        1.{" "}
                        {
                          "Go to your Gitlab -> top-right profile picture -> Preferences"
                        }
                      </span>
                      ...
                      <span>
                        3.{" "}
                        {
                          "Remove expiration date, check read_api checkbox create and copy the token"
                        }
                      </span>
                    </span>
                  )}
                ...
                <div className="flex flex-row flex-wrap items-end mt-4">
                  {/* for each field */}
                  {this.state.selectedDataSource.configFields.map(
                    (field, index) => {
                      if (
                        ...
                        return (
                          <div className="flex flex-col mr-10 mt-4">
                            <h1 className="text-lg block text-black mb-4">
                              {field.label}
                            </h1>
                            <input
                              value={field.value}
                              onChange={(event) => {
                                this.updateInput(index, event.target.value);
                              }}
                              className="w-96 h-10 rounded-lg bg-[rgba(0,0,0,0.06)] text-black p-2"
                        ...
                        return (
                          <div className="flex flex-col w-full mt-4">
                            ...
                            <textarea
                              ...
                              className="w-full h-80 rounded-lg bg-[rgba(0,0,0,0.06)] text-black p-2 mb-5"
                              placeholder={field.placeholder}
                            ></textarea>
                          </div>
                        );
                  ...
                  <div
                    ...
                    className="flex py-2 px-3 mx-2 w-30 h-10 mt-4 flex-row items-center justify-center bg-[#0D7E97]
                                  hover:bg-[#0D7E97] hover:cursor-pointer rounded-lg font-dm-sans leading-[28px] border-[#000] transition duration-300 ease-in-out"
                  >
                    {!this.state.isLoading &&
                      !this.state.selectedDataSource.hasAdditionalSteps && (
                    ...
                    {!this.state.isLoading &&
                      this.state.selectedDataSource.hasAdditionalSteps && (
                        <h1 className="text-black">
                          {this.state.isSelectingLocations
                            ? `Proceed with ${this.state.selectedLocations.length} locatons`
                            : "Proceed"}
                        </h1>
                      )}
                  ...
                  {this.state.isSelectingLocations && (
                    <div
                      onClick={this.indexEverything}
                      className="flex py-2 px-3 mx-2 w-30 h-10 mt-4 flex-row items-center justify-center bg-[rgba(0,0,0,0.06)]
                                  hover:bg-[rgba(0,0,0,0.06)] hover:cursor-pointer rounded-lg font-dm-sans leading-[28px] border-[#0D7E97] transition duration-300 ease-in-out"
                    >
                      <h1 className="text-black">or Index everything</h1>
  ...
  getLocations = () => {
    ...
    toast.info("Looking for locations... (this may take a few seconds)", {
      autoClose: 6000,
    });
    ...
    api
      .post<IndexLocation[]>(
        `/data-sources/${this.state.selectedDataSource.value}/list-locations`,
        config,
        {
          headers: {
      ...
      .catch((error) => {
        toast.dismiss();
        toast.error("Error listing locations: " + error.response.data, {
          autoClose: 10000,
        });
  ...
  submit = () => {
    ...
    api
      ...
      .catch((error) => {
        toast.error("Error adding data source: " + error.response.data, {
          autoClose: 10000,
        });
        this.setState({ isLoading: false });
      });
  ...
  reset = () => {
    this.setState({
      isLoading: false,
      isAdding: false,
      selectedDataSource: this.state.selectOptions[0],
      isSelectingLocations: false,
      selectedLocations: [],
    });
  };
  ...
  removeDataSource = (index: number) => {
    ...
    toast.success(
      `Removing ${this.capitilize(
        connectedDataSource.name
      )}... (it may take some time)`,
      { autoClose: 6000 }
    );
    api
      .delete(`/data-sources/${connectedDataSource.id}`, {
        headers: {
          uuid: localStorage.getItem("uuid"),
        },
      })
      ...
      .catch((error) => {
        toast.error("Error removing data source: " + error.response.data, {
          autoClose: 10000,
        });
      });
  };
